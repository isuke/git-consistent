...
const checkAddedFile = () => {
  const addedFiles = execSync(`git diff --name-only --cached 2> /dev/null`)
    .toString()
    .trim()
...
const replaceTerms = async (program, template, definitions, terms, callback) => {
  await inquirer
    .prompt(createQuestions(program, definitions, terms))
    .then((answers) => {
      let result = template
...
// Main
//
module.exports = async (program, template, definitions, terms) => {
  if (!program.dryRun) checkAddedFile()

  await replaceTerms(program, template, definitions, terms, (commitMessage) => {
